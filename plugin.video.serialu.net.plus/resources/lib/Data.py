...
class Data(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        db = xbmc.translatePath(os.path.join(self.Auth.Data_path, r'serialu_net.db3'))
        #---
        try:
    ...
    #-- get system info
    def get_Info(self):
        self.cur.execute("SELECT Page_Loaded FROM system LIMIT 0, 1")
        self.con.commit()
        return self.cur.fetchone()[0]

    #-- update system info
    def upd_Info(self, page):
        update_date = time.strftime('%Y-%m-%d %H:%M:%S')

        self.cur.execute("UPDATE system SET Page_Loaded = ?, Last_Update = ?", [page, update_date])
        self.con.commit()