...
class MemStorage(MutableMapping):
    ...
    def __setitem__(self, key, value):
        ...
        self._window.setProperty(full_key, pickled)
        if key != '__keys__':
            keys = self['__keys__']
            keys.append(key)
            self['__keys__'] = keys
...
class Addon(object):
    ...
    def __init__(self, id_=''):
        ...
        self._configdir = xbmcvfs.translatePath(self._addon.getAddonInfo('profile'))
    ...
    def get_setting(self, id_, convert=True):
        ...
        if convert:
            if setting == 'true':
                ...
                return int(setting)  # Convert numeric strings to int
