...
import xbmc, xbmcgui, xbmcplugin, xbmcaddon, subprocess

Addon = xbmcaddon.Addon(id='plugin.video.test')
...
def Test(params):
    print '***'
    i = xbmcgui.ListItem('TEST', iconImage=icon, thumbnailImage=icon)
    u = sys.argv[0] + '?mode=PLAY'
    u += '&name=%s'%urllib.quote_plus('TEST')
    u += '&url=%s'%urllib.quote_plus('http://localhost:8000')
    u += '&img=%s'%urllib.quote_plus(icon)
    xbmcplugin.addDirectoryItem(h, u, i, False)
...
def PLAY(params):
    #-- get session UID
    url  = 'http://www.hdserials.ru/Multfilmy/Pixar-Animation-Studios/Hrabraya-serdtsem-/-Brave.html'
    html = get_HTML(url)
    soup = BeautifulSoup(html, fromEncoding="windows-1251")
    UID = re.compile('video\/(.+?)\/iframe').findall(soup.find('iframe')['src'])[0]

    #-- get list of video url
    url = 'http://moonwalk.cc/video/'+UID+'/manifest.f4m'

    # 966 290
    cmd = 'php '+os.path.join(Addon.getAddonInfo('path'), r'AdobeHDS.php')+' --manifest "'+url+'" --quality "290" --play | "C:\Program Files (x86)\VideoLAN\VLC\Vlc.exe" --file-caching=10000 - --fullscreen --play-and-exit --video-on-top --no-video-title-show'
    print cmd
    #os.system(cmd)
    ...
    try:
        ...
        # -- play video
        i = xbmcgui.ListItem(name, url, thumbnailImage=img)
