	...
	def Search(self, text="миля", category=4):
		...
		for t in RootList:
			if len(t)==4 :
				...
				n=size.find(".")
				flt=0
				if size[-4:]=="\xd0\x9c\xd0\xb1" and n>0:
						size=size[:n]
						if len(size)<3: flt=1
						if len(size)==0:size="    ------    "
				...
				if flt==0:
					if qual<>"    ------    ":Lout1.append([qual,size,xt(t[2]),t[3]])
					else:Lout2.append([qual,size,xt(t[2]),t[3]])
					#print xt(t[2])
					Lout=Lout1+Lout2#.append([qual,size,xt(t[2]),t[3]])
		return Lout