...
def doSearch(params):
    ...
    channels=beautifulSoup.findAll(id= re.compile('entryID[0-9]+'))
    for ch in channels: 
        ...
        if tttl: 
            ...
            try:
                li = xbmcgui.ListItem(tttl, iconImage = addon_icon, thumbnailImage = img)
    ...
    gogers=beautifulSoup.findAll('a', attrs={'class': 'swchItem1'})
    for n in gogers: 
        link= n['href']
...
def openitem(params):
    ...
    #print titl.encode('utf-8')
    img= beautifulSoup.findAll('img', attrs={'width':'0'})[0]['src']
    #print img
    hh= re.compile('<b>Оригинальное название:</b>.+')
    #print ch
