# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/cScVOD/parsers/cScParser.py
import re
import urllib2
import urllib
        ...
        try:
            ...
            if url.find('fileplaneta.com') > -1:
                ...
                try:
                    page = urllib2.urlopen(request).read()
                    if(hd == '1'):
                        str = '<id>' + string + '</id>.*?\s.*?<filehd>(.*?)</filehd>' 
                        code = re.findall(str, page)
                        code = code[0]
                        if len(code) > 0:
                            url = code
                    else:
            ...
            if url.find('ofx.xyz') > -1 or url.find('85.195.88.135') > -1:
                request = urllib2.Request(url, None, {'User-agent': 'Mozilla/5.0 nStreamVOD 0.1',
                 'Connection': 'Close'})
                try:
                    page = urllib2.urlopen(request).read()
